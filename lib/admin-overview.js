...
const { loadLegalDocs } = require('./legal-docs');
const { summarizeTodayAccess } = require('./access-log');
const { getNotesCoverage } = require('./gir-notes');
const { listTaxonomySummary } = require('./material-taxonomy');

let customsTypesCount = 0;
let glossaryCount = 0;
try {
  customsTypesCount = Object.keys(require('../data/customs-types.json').types || {}).length;
} catch {
  customsTypesCount = 0;
}
try {
  glossaryCount = require('../data/glossary-xnk.json').entryCount || 0;
} catch {
  glossaryCount = 0;
}
...
function buildAdminOverview() {
  ...
  return {
    ...
    todayStats: summarizeTodayAccess(),
    ...
    knowledgeLayers: {
      ...
      materialTaxonomyFamilies: listTaxonomySummary(),
    },
    dataCatalog: {
      customsTypes: customsTypesCount,
      legalDocuments: loadLegalDocs().total || Object.keys(loadLegalDocs().documents || {}).length,
      glossaryEntries: glossaryCount,
      isoCountries: require('./compliance-data').iso3166Data.countries?.length || 0,
      tchqUnits: require('./compliance-data').unitsTchqData.units?.length || 0,
    },
    apiEndpoints: {
      search: '/api/search',
      suggest: '/api/suggest',
      ...
      tax: '/api/tax',
      customsTypes: '/api/customs-types',
      legalDocs: '/api/legal-docs',
      match: 'POST /api/match',
      materials: '/api/materials',
      ministries: '/api/ministries',
      tariffVersions: '/api/versions',
      tariffSnapshot: 'POST /api/tariff?op=snapshot',
      tariffRollback: 'POST /api/tariff?op=rollback',
    },
    recentUpdates: {
