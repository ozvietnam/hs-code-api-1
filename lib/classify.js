...
4. Dùng GỒM / KHÔNG GỒM / LOẠI TRỪ + TÍNH CHẤT + PHÂN BIỆT làm ĐIỂM KIỂM TRA:
...
function buildContext(attrs, headings) {
  ...
  for (const h4 of headings) {
    ...
    if (n.chuong) chapLines.push(`■ Chương ${ch2}: ${clip(n.chuong, 550)}`);
  }
  if (chapLines.length) {
  ...
  for (const h4 of headings) {
    const n = headingNotes(h4);
    lines.push(`\n■ Nhóm ${h4}:`);
    if (n.nhom) lines.push(`  Chú giải nhóm: ${clip(n.nhom, 680)}`);
    if (n.baoGom) lines.push(`  ✅ GỒM: ${clip(n.baoGom, 340)}`);
    if (n.khongBaoGom) lines.push(`  ⛔ KHÔNG GỒM: ${clip(n.khongBaoGom, 260)}`);
    if (n.loaiTru) lines.push(`  ⛔ LOẠI TRỪ: ${clip(n.loaiTru, 180)}`);
    if (n.tinhChat) lines.push(`  Tính chất điển hình: ${clip(n.tinhChat, 160)}`);
    if (n.phanBiet) lines.push(`  Phân biệt: ${clip(n.phanBiet, 180)}`);
    for (const c of codesInHeading(h4, 12)) lines.push(`  - ${c.hs}: ${clip(c.vn, 70)}`);
  }
  return lines.join('\n');
}
